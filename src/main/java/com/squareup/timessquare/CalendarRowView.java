...
public class CalendarRowView extends ViewGroup implements View.OnClickListener {
  ...
  @Override public void addView(View child, int index, ViewGroup.LayoutParams params) {
    ((FrameLayout)child).getChildAt(0).setOnClickListener(this);
  ...
  public void onClick(View v) {
    ...
    if (listener != null) {
      listener.handleClick((MonthCellDescriptor)
              //((FrameLayout) v).getChildAt(0).getTag()
              v.getTag()
      );
    }
  }
  ...
  public void setCellBackground(int resId) {
    for (int i = 0; i < getChildCount(); i++) {
      ((FrameLayout)getChildAt(i)).getChildAt(0).setBackgroundResource(resId);
    }
  }

  public void setCellTextColor(int resId) {
    for (int i = 0; i < getChildCount(); i++) {
      ((TextView)(((FrameLayout)getChildAt(i)).getChildAt(0))).setTextColor(resId);
    }
  }

  public void setCellTextColor(ColorStateList colors) {
    for (int i = 0; i < getChildCount(); i++) {
      ((TextView)(((FrameLayout)getChildAt(i)).getChildAt(0))).setTextColor(colors);
    }
  }

  public void setTypeface(Typeface typeface) {
    for (int i = 0; i < getChildCount(); i++) {
      ((TextView)(((FrameLayout)getChildAt(i)).getChildAt(0))).setTypeface(typeface);
    }
  }
}
